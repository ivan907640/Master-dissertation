...
def test(epoch, checkpoint, data_test, label_test, n_classes):
    ...
    AUROCs, Accus, Pre, Recall = epochVal_metrics_test(model, test_dl, args.model, n_classes=n_classes) #thresh=0.4
...
if __name__ == '__main__':
    ...
    time_current = 'attempt1'
    if args.log_file_name is None:
        args.log_file_name = 'log-%s' % (datetime.datetime.now().strftime("%m-%d-%H%M-%S"))
    log_path = args.log_file_name + '.log'
    filepth=os.path.join(args.logdir, log_path)
    logging.basicConfig(filename=filepth, level=logging.INFO,
                        format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
    logger = logging.getLogger()
    logger.addHandler(logging.StreamHandler(sys.stdout))
    logger.addHandler(logging.FileHandler(filepth,mode='a',encoding='utf-8'))
    ...
    for com_round in trange(start_epoch, args.rounds):
        ...
        for meta_round in range(args.meta_round):
            clt_list_this_meta_round = random.sample(list(range(1, total_num)), args.meta_client_num-1)
            clt_list_this_meta_round.append(0)
            print(clt_list_this_meta_round)
        ...
        AUROC_avg, Accus_avg, Pre_avg, Recall_avg = test(com_round, net_glob.state_dict(), X_test, y_test, n_classes)
        writer.add_scalar('AUC', AUROC_avg, global_step=com_round)
        writer.add_scalar('Acc', Accus_avg, global_step=com_round)
        writer.add_scalar('AUC', Pre_avg, global_step=com_round)
        writer.add_scalar('Acc', Recall_avg, global_step=com_round)
        logger.info("\nTEST Student: Epoch: {}".format(com_round))
        logger.info("\nTEST AUROC: {:6f}, TEST Accus: {:6f}, TEST Pre: {:6f}, TEST Recall: {:6f}"
                    .format(AUROC_avg, Accus_avg, Pre_avg, Recall_avg))
